...
function ScrollGlobe({ sections, globeConfig = defaultGlobeConfig, className }: ScrollGlobeProps) {
  ...
  return (
    <div 
      ref={containerRef}
      className={cn(
        "relative w-full max-w-screen overflow-x-hidden min-h-screen bg-background text-foreground pt-16",
        className
      )}
    >
      {/* Progress Bar */}
      <div className="fixed top-16 left-0 w-full h-0.5 bg-gradient-to-r from-border/20 via-border/40 to-border/20 z-40">
        <div 
      ...
      {sections.map((section, index) => (
        <section
          ...
          className={cn(
            ...
            (!section.align || (section.align !== 'center' && section.align !== 'right')) && "items-start text-left"
          )}
        >
          <div className={cn(
  ...
  const demoSections = [
    {
      ...
      badge: "Welcome",
      title: "ServiceSwap",
      subtitle: "Digital Second-Hand Services Exchange",
      description: "Transform unused services into valuable opportunities. Whether it's a Netflix subscription, gym membership, or online course - connect with others to share, transfer, and maximize every service you've paid for.",
      align: "left" as const,
      actions: [
        { label: "Get Started", variant: "primary" as const, onClick: () => window.location.href = '/login' },
        { label: "Learn More", variant: "secondary" as const, onClick: () => window.location.href = '/about' },
      ]
    },
    {
      id: "innovation",
      badge: "Revolution",
      title: "Redefining Service Sharing",
      description: "India's first marketplace for unused services and subscriptions. From Netflix slots to gym memberships, course access to software licenses - we're building a connected ecosystem where every service finds its perfect match.",
    ...
    },
    {
      ...
      badge: "Features",
      title: "Smart & Secure",
      subtitle: "Service Exchange",
      description: "Experience the future of service sharing with our comprehensive platform designed for safety, convenience, and maximum value extraction from your unused subscriptions and memberships.",
      align: "left" as const,
      features: [
        { title: "Escrow Protection", description: "Secure payment system ensuring safe transfers for both buyers and sellers" },
        { title: "Verified Listings", description: "Premium verification system to guarantee authentic services and trusted users" },
        { title: "Smart Matching", description: "AI-powered recommendations to connect you with perfect service opportunities" }
      ]
    },
    {
      id: "future",
      ...
      description: "Be part of India's growing community of smart service sharers. Turn your unused subscriptions into income, discover amazing deals on premium services, and help build a more efficient sharing economy.",
      align: "center" as const,
      actions: [
        { label: "Join Now", variant: "primary" as const, onClick: () => window.location.href = '/login' },
        { label: "Learn More", variant: "secondary" as const, onClick: () => window.location.href = '/about' }
      ]
    }
  ];
